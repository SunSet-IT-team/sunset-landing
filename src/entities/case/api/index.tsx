...
import { CaseAPIMethods } from './types';
...
const CaseAPI: CaseAPIMethods = {
    getCases: async (params: {
        ...
        if (!res.ok) {
            const error = new Error(data.message || 'Ошибка при запросе кейсов') as any;
    ...
    getCasesData: async (fields, cache: boolean = true) => {
        ...
        if (!res.ok) {
            const error = new Error(slugs.message || 'Ошибка при запросе кейсов') as any;
